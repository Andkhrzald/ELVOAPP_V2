        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-12">
            ...
            <div class="space-y-1.5 px-1">
                ...
                <div class="flex items-center gap-1">
                    @for($i = 1; $i <= 5; $i++)
                        <svg class="w-3 h-3 {{ $i <= round($avgRating) ? 'text-yellow-500' : 'text-gray-700' }}" fill="currentColor" viewBox="0 0 20 20">
...
<script>
    function getCart() {
        return JSON.parse(localStorage.getItem('elvo_cart') || '[]');
    }

    function saveCart(cart) {
        localStorage.setItem('elvo_cart', JSON.stringify(cart));
    }

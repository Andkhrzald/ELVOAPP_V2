...
<div class="min-h-screen bg-[#0f0f0f] text-gray-300 pb-12">
    ...
    <div class="max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between mb-8 px-4 lg:px-0 pt-10 gap-4">
        <div>
            ...
            <p class="text-[11px] font-semibold text-blue-500 uppercase tracking-[0.2em] mt-1">Rating & Ulasan Produk</p>
        </div>
        <form action="{{ route('admin.reviews') }}" method="GET" class="flex gap-3">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari review..."
                class="bg-[#1a1a1a] border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-gray-600 w-56 focus:outline-none focus:border-blue-500/50">
    ...
    <div class="max-w-7xl mx-auto px-4 lg:px-0 mb-6 flex gap-3 flex-wrap">
        <a href="{{ route('admin.reviews') }}" class="px-4 py-2 rounded-xl text-xs font-bold transition {{ !request('rating') ? 'bg-yellow-500 text-black' : 'bg-[#1a1a1a] text-gray-400 border border-white/5' }}">Semua</a>
        @for($i = 5; $i >= 1; $i--)
        <a href="{{ route('admin.reviews', ['rating' => $i]) }}" class="flex items-center gap-1 px-4 py-2 rounded-xl text-xs font-bold transition {{ request('rating') == $i ? 'bg-yellow-500 text-black' : 'bg-[#1a1a1a] text-gray-400 border border-white/5' }}">
    ...
    <div class="max-w-7xl mx-auto px-4 lg:px-0 space-y-4">
        ...
        <div class="bg-[#1a1a1a] rounded-2xl border border-white/5 p-6 hover:border-yellow-500/10 transition">
            <div class="flex flex-col md:flex-row md:items-start justify-between gap-4">
                ...
                <form action="{{ route('admin.reviews.destroy', $review->id) }}" method="POST" onsubmit="return confirm('Hapus review ini?')">
                    @csrf @method('DELETE')
                    <button class="px-3 py-1.5 bg-red-500/10 text-red-500 border border-red-500/20 rounded-lg text-[10px] font-bold uppercase hover:bg-red-500 hover:text-white transition">Hapus</button>
                </form>
            </div>
        </div>
        @empty

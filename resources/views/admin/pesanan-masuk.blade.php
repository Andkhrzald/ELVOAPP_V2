...
<div class="min-h-screen bg-[#0f0f0f] text-gray-300 pb-12">
    ...
    <div class="max-w-[1600px] mx-auto pt-10 px-6 lg:px-10 mb-12">
        <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            <div>
                <p class="text-[10px] font-black text-blue-500 uppercase tracking-[0.4em] mb-3">Order Management / Logistic Hub</p>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div class="bg-[#1a1a1a] p-5 rounded-2xl border border-white/5 flex flex-col items-center justify-center text-center group hover:border-blue-500/30 transition-all">
                    <p class="text-[9px] font-black text-gray-500 uppercase tracking-widest mb-1 group-hover:text-blue-500">Total Orders</p>
                    <p class="text-2xl font-black text-white leading-none">{{ $totalOrders }}</p>
                </div>
                <div class="bg-[#1a1a1a] p-5 rounded-2xl border border-white/5 flex flex-col items-center justify-center text-center group hover:border-orange-500/30 transition-all">
                ...
                <div class="hidden md:flex bg-blue-600 p-5 rounded-2xl shadow-xl shadow-blue-900/20 flex-col items-center justify-center text-center">
                    <p class="text-[9px] font-black text-blue-100 uppercase tracking-widest mb-1">Status Aktif</p>
    ...
    <div class="max-w-[1600px] mx-auto px-6 lg:px-10 grid grid-cols-12 gap-8">
        ...
        <div class="col-span-12 lg:col-span-3 space-y-6">
            <div class="bg-[#1a1a1a] p-6 rounded-[2rem] border border-white/5 sticky top-24">
                    ...
                    <div>
                        ...
                        <div class="relative group">
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Order ID / Nama / HP" 
                                class="w-full bg-[#121212] border border-white/5 rounded-xl px-5 py-3 text-sm text-white focus:border-blue-500 outline-none transition-all placeholder-gray-700">
                            <button type="submit" class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-blue-500 transition-colors">
        ...
        <div class="col-span-12 lg:col-span-9 space-y-6">
            ...
            <div class="group bg-[#1a1a1a] rounded-[2rem] border border-white/5 overflow-hidden hover:border-blue-500/20 transition-all shadow-2xl relative">
                {{-- Glowing indicator --}}
                <div class="absolute top-0 left-0 bottom-0 w-1 {{ $status == 'pending' ? 'bg-blue-500' : ($status == 'proses' ? 'bg-purple-500' : ($status == 'selesai' ? 'bg-green-500' : 'bg-gray-700')) }}"></div>

                {{-- Order Header --}}
                <div class="p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-white/5">
                    <div class="flex items-center gap-5">
                        <div class="w-14 h-14 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-center text-xl font-black text-white group-hover:bg-blue-600 group-hover:border-blue-500 transition-all duration-500">
                            {{ substr($order->user->name ?? 'U', 0, 1) }}
                        </div>
                        <div>
                            <div class="flex items-center gap-3">
                                <h3 class="text-lg font-black text-white leading-none">{{ $order->user->name ?? 'Unknown Customer' }}</h3>
                                <span class="px-2 py-0.5 bg-blue-500/10 text-blue-500 text-[8px] font-black uppercase tracking-widest rounded border border-blue-500/20">Verified Buyer</span>
                            </div>
                ...
                <div class="p-8 grid grid-cols-1 md:grid-cols-3 gap-10">
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg bg-[#121212] flex items-center justify-center text-xs text-gray-500 border border-white/5">PH</div>
                                <p class="text-xs font-bold text-gray-300">{{ $order->user->phone ?? '-' }}</p>
                            </div>
                            <div class="flex items-start gap-3">
                                <div class="w-8 h-8 rounded-lg bg-[#121212] flex items-center justify-center text-xs text-gray-500 border border-white/5 shrink-0">AD</div>
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center gap-3 group/item">
                                <div class="w-10 h-10 rounded-xl bg-white/5 overflow-hidden border border-white/5 flex-shrink-0">
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] text-gray-600 font-bold uppercase tracking-tighter">{{ $item->quantity }} Units · <span class="text-blue-500/60">Rp {{ number_format($item->price, 0, ',', '.') }}</span></p>
                                </div>
                            </div>
                            @endforeach
                        </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-4">
                            <div class="bg-[#121212] p-3 rounded-xl border border-white/5">
                            ...
                            <div class="bg-[#121212] p-3 rounded-xl border border-white/5">
                                ...
                                <p class="text-[10px] font-black text-blue-500 uppercase">{{ $order->shipping_method ?? 'PENDING ASSIGNMENT' }}</p>
                            </div>
                            @if($order->no_resi)
                            <div class="bg-blue-500/10 p-3 rounded-xl border border-blue-500/20">
                                <p class="text-[8px] font-black text-blue-500 uppercase mb-1">Tracking Number</p>
                ...
                {{-- Action Bar --}}
                <div class="px-8 py-5 bg-white/[0.02] border-t border-white/5 flex flex-wrap items-center justify-between gap-4">
                    <div class="flex items-center gap-4">
                        ...
                        <form action="{{ route('admin.orders.accept', $order->id) }}" method="POST">
                            @csrf
                            <button class="px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-xl shadow-blue-900/20">
                                Confirm & Process
                            </button>
                        </form>
                        @endif
                        ...
                        <form action="{{ route('admin.orders.complete', $order->id) }}" method="POST">
                            @csrf
                            <button class="px-6 py-2.5 bg-green-600 hover:bg-green-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-xl shadow-green-900/20">
                                Confirm Receipt
                            </button>
                        </form>
                        @endif
                        ...
                        <div class="flex gap-2">
                            <form action="{{ route('admin.orders.confirm-cancel', $order->id) }}" method="POST">
                                @csrf
                                <button class="px-6 py-2.5 bg-red-600 hover:bg-red-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all">Accept Cancellation</button>
                            </form>
                            <form action="{{ route('admin.orders.reject-cancel', $order->id) }}" method="POST">
                                @csrf
                                <button class="px-6 py-2.5 bg-white/5 hover:bg-white/10 text-gray-400 text-[10px] font-black uppercase tracking-widest rounded-xl border border-white/5 transition-all">Decline Request</button>
                            </form>
                        </div>
                        @endif
                        ...
                        <div class="flex gap-2">
                            ...
                            <form action="{{ route('admin.orders.reject-refund', $order->id) }}" method="POST">
                                @csrf
                                <button class="px-6 py-2.5 bg-white/5 hover:bg-white/10 text-gray-400 text-[10px] font-black uppercase tracking-widest rounded-xl border border-white/5 transition-all">Decline Refund</button>
                            </form>
                        </div>
                        @endif
            ...
            @empty
            <div class="bg-[#1a1a1a] rounded-[2rem] border border-white/5 p-20 text-center">
                <div class="w-20 h-20 bg-[#121212] rounded-full flex items-center justify-center mx-auto mb-6">
...
<div id="ship-modal" class="fixed inset-0 z-[200] hidden items-center justify-center bg-black/80 backdrop-blur-md">
    <div class="relative p-10 w-full max-w-lg bg-[#1a1a1a] rounded-[2.5rem] shadow-2xl border border-white/10 animate-modal-in">
        ...
        <form id="ship-form" method="POST" class="space-y-8">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <div class="relative group">
                        <select name="shipping_method" class="w-full bg-[#121212] border border-white/5 rounded-2xl px-5 py-4 text-sm font-bold text-white focus:border-purple-500 outline-none transition-all appearance-none">
                ...
                <div>
                    ...
                    <input type="text" name="no_resi" required placeholder="Enter AWB Number..." 
                        class="w-full bg-[#121212] border border-white/5 rounded-2xl px-5 py-4 text-sm font-black text-white focus:border-purple-500 outline-none transition-all placeholder-gray-800 font-mono tracking-widest">
                </div>
            </div>

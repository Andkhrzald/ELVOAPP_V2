...
<div class="mb-6">
    <div class="flex items-center gap-2 mb-1">
        <h1 class="text-2xl font-bold animate-fade-up animate-fade-up text-white">Manajemen Admin</h1>
...
{{-- Form Tambah Admin --}}
<div class="p-6 bg-elvo-surface rounded-xl border border-white/[0.06] shadow-[0_0_25px_rgba(0,0,0,0.2)] mb-6 card-hover">
    ...
    <form action="{{ route('admin.owner.manage-admins.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div>
            ...
            <input type="text" name="name" required
                class="w-full px-3 py-2 text-sm border border-white/10 rounded-lg bg-[#252525] text-white focus:ring-elvo-primary focus:border-elvo-primary"
        ...
        <div class="flex items-end">
            <button type="submit"
                class="w-full px-4 py-2 text-sm font-semibold btn-primary rounded-lg">
                + Tambah Akun
            </button>
        </div>
    </form>
                ...
                <tr class="border-b border-white/[0.06] hover:bg-white/5">
                    <td class="py-3 text-white">
                        <div class="flex items-center gap-2">
                            <div class="w-7 h-7 rounded-full bg-elvo-primary flex items-center justify-center text-xs text-white font-bold">
                                {{ strtoupper(substr($admin->name, 0, 1)) }}
                            </div>
                            {{ $admin->name }}
                            @if($admin->id === auth()->id())
                            <span class="text-[10px] text-[#8b7df2] font-semibold">(Anda)</span>
                            @endif
                        </div>
                    </td>
                    ...
                    <td class="py-3">
                        ...
                        @else
                        <span class="px-2 py-0.5 text-[10px] font-bold text-elvo-primary bg-elvo-primary/10 rounded-full">ADMIN</span>
                        @endif
                    </td>

...
<div class="min-h-screen text-gray-300 pb-12">
    ...
    <div class="max-w-6xl mx-auto flex items-end justify-between mb-10 px-4 lg:px-0 pt-6">
        <div>
            <p class="text-[10px] font-black text-elvo-primary uppercase tracking-[0.3em] mb-2">Management / Inventory / Edit</p>
            <h1 class="text-4xl font-black text-white tracking-tighter">Edit Data Produk</h1>
        </div>
        <a href="{{ route('admin.products') }}" class="group flex items-center gap-3 text-[10px] font-black text-gray-500 hover:text-white transition-all uppercase tracking-widest bg-white/5 px-4 py-2 rounded-xl border border-white/[0.06]">
    ...
    <div class="max-w-6xl mx-auto px-4 lg:px-0">
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-12 gap-8">
            ...
            <div class="col-span-12 lg:col-span-8 space-y-8">
                {{-- Main Info Card --}}
                <div class="bg-elvo-surface p-10 rounded-[2.5rem] border border-white/[0.06] shadow-2xl relative overflow-hidden">
                    ...
                    <div class="space-y-10 relative z-10">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $product->name) }}" id="productName" onkeyup="generateSlug(this.value)"
                                class="w-full bg-transparent px-0 py-2 text-3xl font-black text-white border-b-2 border-white/10 focus:border-elvo-primary outline-none transition-all placeholder-white/10" required>
                        </div>

                        <div class="bg-elvo-bg p-4 rounded-2xl border border-white/[0.06] flex items-center gap-2">
                            ...
                            <input type="text" name="slug" id="productSlug" value="{{ old('slug', $product->slug) }}" class="bg-transparent border-none p-0 focus:ring-0 text-[10px] font-black text-elvo-primary font-mono w-full" readonly>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                            ...
                            <div class="md:col-span-1">
                                ...
                                <div class="relative group">
                                    <select name="category_id" class="w-full bg-elvo-bg py-4 px-6 rounded-2xl text-sm font-bold border border-white/[0.06] focus:border-elvo-primary outline-none cursor-pointer appearance-none transition-all">
                                        @foreach($categories as $cat)
                                            <option value="{{ $cat->id }}" {{ $product->category_id == $cat->id ? 'selected' : '' }} class="bg-elvo-surface">
                                                {{ $cat->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <div class="absolute inset-y-0 right-6 flex items-center pointer-events-none text-gray-500 group-hover:text-elvo-primary transition-colors">
                            ...
                            <div class="md:col-span-1">
                                ...
                                <input type="text" name="color" value="{{ old('color', $product->color) }}" 
                                    class="w-full bg-elvo-bg py-4 px-6 rounded-2xl text-sm font-bold border border-white/[0.06] focus:border-elvo-primary outline-none text-white transition-all" placeholder="Contoh: Onyx Black">
                            </div>
                            {{-- Berat --}}
                            <div class="md:col-span-1">
                                ...
                                <div class="relative">
                                    <input type="number" name="weight" value="{{ old('weight', $product->weight) }}" 
                                        class="w-full bg-elvo-bg py-4 px-6 rounded-2xl text-sm font-bold border border-white/[0.06] focus:border-elvo-primary outline-none text-white transition-all" required>
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="6" 
                                class="w-full bg-elvo-bg px-8 py-6 text-sm text-gray-300 rounded-[2rem] border border-white/[0.06] focus:border-elvo-primary outline-none resize-none transition-all leading-relaxed shadow-inner" required>{{ old('description', $product->description) }}</textarea>
                        </div>
                    </div>
                </div>

                {{-- Media Card --}}
                <div class="bg-elvo-surface p-10 rounded-[2.5rem] border border-white/[0.06] shadow-2xl">
                    ...
                    <div class="grid grid-cols-1 md:flex items-center gap-10">
                        ...
                        <label class="flex-1 flex flex-col items-center justify-center h-48 border-2 border-dashed border-white/[0.06] rounded-[2rem] cursor-pointer hover:bg-elvo-primary/[0.03] hover:border-elvo-primary/50 transition-all group">
                            <div class="bg-elvo-primary/10 p-4 rounded-full mb-4 group-hover:scale-110 transition-transform">
                                <svg class="w-8 h-8 text-elvo-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 4v16m8-8H4" stroke-width="3" stroke-linecap="round"/></svg>
                            </div>
            ...
            <div class="col-span-12 lg:col-span-4 space-y-8">
                {{-- Pricing Card --}}
                <div class="bg-elvo-primary p-10 rounded-[2.5rem] shadow-2xl shadow-[#7c6df0]/30 relative overflow-hidden group">
                    {{-- Decorative SVG --}}
                    ...
                    <div class="relative z-10">
                        <label class="block text-[10px] font-black text-gray-300 uppercase tracking-[0.2em] mb-6">Valuation / Harga Jual</label>
                        ...
                        <p class="text-[9px] text-gray-400 mt-4 uppercase font-bold tracking-widest">Base price in Indonesian Rupiah</p>
                    </div>
                </div>
                ...
                <div class="bg-elvo-surface p-10 rounded-[2.5rem] border border-white/[0.06] shadow-2xl">
                    <div class="mb-10">
                        ...
                        <div class="flex items-center gap-5 bg-elvo-bg p-6 rounded-2xl border border-white/[0.06] focus-within:border-elvo-primary transition-all shadow-inner">
                    ...
                    <div class="space-y-4">
                        <button type="submit" class="w-full py-5 bg-white text-black text-[10px] font-black uppercase tracking-[0.3em] rounded-2xl hover:bg-elvo-primary hover:text-white transition-all shadow-xl shadow-black/40">
                            Simpan Perubahan
                        </button>
                ...
                {{-- Visibility Status --}}
                <div class="bg-elvo-surface p-8 rounded-[2rem] border border-white/[0.06] shadow-2xl">
                ...
                <div class="p-6 border border-red-500/20 rounded-[2rem] bg-red-500/5">
                    ...
                    <button type="button" onclick="confirmDelete()" class="text-[9px] font-black btn-danger px-4 py-2 rounded-lg uppercase tracking-tighter">

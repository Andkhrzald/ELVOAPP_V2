...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-400">Kelola kategori untuk pengelompokan produk.</p>
        </div>
    ...
    <div class="bg-elvo-surface rounded-xl border border-white/[0.06] shadow-[0_0_25px_rgba(0,0,0,0.2)] overflow-hidden">
        <table class="w-full text-left">
            <thead>
                <tr class="bg-white/[0.02] border-b border-white/[0.06]">
                    <th class="px-6 py-4 text-[10px] font-black text-gray-500 uppercase tracking-widest">Nama Kategori</th>
                    <th class="px-6 py-4 text-[10px] font-black text-gray-500 uppercase tracking-widest">Slug</th>
                    <th class="px-6 py-4 text-[10px] font-black text-gray-500 uppercase tracking-widest text-center">Jumlah Produk</th>
            ...
            <tbody class="divide-y divide-white/5">
                ...
                <tr class="hover:bg-white/[0.01] transition-colors">
                    <td class="px-6 py-4 text-sm font-bold text-white">{{ $cat->name }}</td>
                    <td class="px-6 py-4 text-sm text-gray-400 font-mono">{{ $cat->slug }}</td>
                    <td class="px-6 py-4 text-center">
                        <span class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-white/5 text-xs font-bold text-gray-300">{{ $cat->products_count }}</span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex justify-end items-center gap-2">
                            <button onclick="editCategory({{ $cat->id }}, '{{ $cat->name }}')" class="p-2 bg-white/5 text-gray-400 hover:text-white hover:bg-elvo-primary/20 rounded-lg transition-all">
        ...
        <div class="p-6 border-t border-white/[0.06]">{{ $categories->links() }}</div>
        @endif
    </div>
</div>
    ...
    <div class="relative p-8 w-full max-w-lg bg-elvo-surface rounded-[2rem] shadow-2xl border border-white/10 animate-modal-in">
        ...
        <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" required class="w-full bg-elvo-bg border border-white/[0.06] rounded-xl px-5 py-3 text-white focus:border-elvo-primary outline-none transition-all" placeholder="Contoh: Hoodie, T-Shirt, Aksesoris">
            </div>
